...
app/                — Next.js 프론트엔드 (App Router)
  ├── quiz/         — 퀴즈 & 페르소나 페이지 (지윤)
  ├── debate/       — 페르소나 디베이트 페이지 (재림)
  ├── plan/         — 90일 실행 플랜 페이지 (석빈)
  ├── market/       — 시장 인사이트 페이지 (동근)
  ├── resources/    — 외부 데이터 허브 페이지 (동근)
  └── api/
      ├── personas/ — 페르소나 생성 API (지윤)
      ├── debate/   — 디베이트 대화 API (재림)
      ├── plan/     — 90일 플랜 생성 API (석빈)
      ├── market/   — 시장 데이터 API (동근)
      ├── resources/— 외부 리소스 API (동근)
      └── health/   — 헬스체크 API
lib/
  ├── market.ts     — 시장 데이터 타입·유틸 (동근)
  ├── resources.ts  — 외부 리소스 타입·유틸 (동근)
  ├── sessions.ts   — Supabase 세션 CRUD (공용)
  └── supabase.ts   — Supabase 클라이언트 (공용)
backend/            — FastAPI 서버
...
## Team ownership (충돌 방지)

각 팀원이 담당하는 디렉토리/파일입니다. 다른 팀원의 영역을 수정할 때는 사전 소통 필수.

| 팀원   | 담당 영역                                                                 |
| ------ | ------------------------------------------------------------------------- |
| 지윤   | `app/quiz/`, `app/api/personas/` — 퀴즈 폼 + 페르소나 생성              |
| 재림   | `app/debate/`, `app/api/debate/` — 페르소나 대화 + 인사이트 추출         |
| 석빈   | `app/plan/`, `app/api/plan/` — 90일 실행 플랜 생성 + 타임라인            |
| 동근   | `app/market/`, `app/resources/`, `app/api/market/`, `app/api/resources/`, `lib/market.ts`, `lib/resources.ts` |
| 공용   | `app/page.tsx`, `app/layout.tsx`, `lib/supabase.ts`, `lib/sessions.ts`, `CLAUDE.md` |
