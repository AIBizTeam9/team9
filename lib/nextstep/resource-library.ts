// 검증된 외부 리소스 카탈로그. 같은 데이터를 두 곳이 쓴다:
//   1) /api/resources GET — 리소스 페이지 UI
//   2) /api/generate-plan — 플랜 생성 시 LLM이 "선택"하는 후보 풀
//
// 핵심 원칙: 모든 URL은 사람이 한 번 손으로 확인한 것만 들어온다. LLM은
// 카탈로그에서 id를 골라 why만 만들고, 서버가 id → real url로 매핑한다.
// 모델이 만든 url은 절대로 그대로 응답에 들어가지 않는다.
...
// 빠른 조회 (id → item)
const BY_ID = new Map<string, ResourceItem>(
  RESOURCE_LIBRARY.map((item) => [item.id, item]),
);
...
// LLM에 보낼 카탈로그 문자열. URL은 일부러 제외 — 모델이 만지지 못하게.
// 형태: `${id} · ${category} · ${title} — ${description}` 한 줄씩.
export function libraryCatalogForPrompt(): string {
  return RESOURCE_LIBRARY.map(
